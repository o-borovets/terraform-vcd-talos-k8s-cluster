locals {
  allow_scheduling_on_control_plane = (local.worker_sum == 0)
    ...
    for node in vcd_vapp_vm.control_plane : node.name => {
      machine = {
        ...
        nodeLabels = merge(
          ...
          local.control_plane_nodepools_map[node.metadata.nodepool].labels
        )
        nodeAnnotations = local.control_plane_nodepools_map[node.metadata.nodepool].annotations
        nodeTaints = {
          for taint in local.control_plane_nodepools_map[node.metadata.nodepool].taints : taint.key => "${taint.value}:${taint.effect}"
        }
        ...
        network = {
          interfaces = concat(
            ...
            [{
              ...
              addresses = ["${node.network[0].ip}/${split("/", local.network_ipv4_cidr)[1]}"]
  ...
  worker_talos_config_patch = {
    for node in vcd_vapp_vm.worker : node.name => {
      machine = {
        install = {
          image           = local.talos_installer_image_url
          extraKernelArgs = var.talos_extra_kernel_args
        }
        nodeLabels      = local.worker_nodepools_map[node.metadata.nodepool].labels
        nodeAnnotations = local.worker_nodepools_map[node.metadata.nodepool].annotations
        certSANs        = local.certificate_san
        network = {
          interfaces = concat(
            local.talos_public_interface_enabled ? [{
...
data "talos_machine_configuration" "control_plane" {
  for_each = { for node in vcd_vapp_vm.control_plane : node.name => node }
...
data "talos_machine_configuration" "worker" {
  for_each = { for node in vcd_vapp_vm.worker : node.name => node }
